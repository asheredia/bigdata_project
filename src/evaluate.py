...
def plot_confusion_matrices_by_snr(model, X_test, Y_test, snr_test, snr_values, class_labels, show_report=False):
    accuracies = []
    confusion_matrices = {}
    ...
    for snr in snr_values:
        ...
        if show_report:
            ...
            report = classification_report(y_true, y_pred, target_names=class_labels, zero_division=0.0)
            print("="*40)
            print(f"Classification Report for SNR {snr} dB")
            print("="*40)
            print(report)
...
def evaluate_model_by_snr(model_path, class_labels, snr_test, snr_values, data_gen, X_test, Y_test, custom_objects=None, show_report=False):
    ...
    results = plot_confusion_matrices_by_snr(
        model, X_test, Y_test, snr_test, snr_values, class_labels, show_report
    )
